...
import HR24Scene from '@/components/hr24-scene';
...
export default function HR24Page(){const [selected,setSelected]=useState('compressor'),[view,setView]=useState('rear'),[transparent,setTransparent]=useState(true),[flow,setFlow]=useState(false),[door,setDoor]=useState(false),[page,setPage]=useState(7);const entry=entries[selected]||entries.cabinet;const figure=`/hr24/${page===7?'construction':page===8?'dimensions':'refrigeration'}.png`,hasFigure=useLocalFile(figure);
 function select(id:string){if(entries[id]){setSelected(id);setPage(entries[id].page);}}
 return <main className="hr-app"><header><div><small>MANUFACTURER SERVICE DRAWINGS</small><h1>Hoshizaki <b>HR24B</b></h1></div><nav><Link href="/reference">이전 개념 모델</Link><a href="/hr24/design.zip" download>CAD·근거 자료 받기</a></nav></header><div className="hr-summary">1도어 · R600a <span>서비스 매뉴얼 73229 / 2021-04-20 개정 / 7–9쪽</span><b>도면 배치 + 업계 일반 부품 형상</b></div>
 ...
 <section className="hr-model"><div className="hr-tools"><select aria-label="제품 시점" value={view} onChange={e=>setView(e.target.value)}><option value="rear">후면 입체</option><option value="back">후면 정면</option><option value="front">전면 입체</option><option value="left">좌측면</option><option value="machine">기계실 확대</option></select><button className={transparent?'active':''} onClick={()=>setTransparent(!transparent)}>외함 투명</button><button onClick={()=>setDoor(!door)}>문 {door?'닫기':'열기'}</button></div><HR24Scene selected={selected} onSelect={select} view={view} transparent={transparent} flow={flow} door={door}/><div className="hr-model-footer"><strong>{entry.name}</strong><span>드래그 회전 · 스크롤 확대 · 부품 선택</span></div><div className="hr-options"><label><input type="checkbox" checked={flow} onChange={e=>setFlow(e.target.checked)}/> 운전 표시 (냉매 흐름 · 증발기 팬)</label></div><p className="hr-note">부품 위치와 냉매 회로 순서는 제조사 도면(7~9쪽)을 따르고, 부품 모양·관경은 이 급 R600a 업소용 냉장고에 흔히 쓰는 부품 치수로 만들었습니다. 벽 속에 묻힌 배관(둘레 액관, 열교환부, 흡입관)은 외함을 투명하게 하면 보입니다. 제조사 원본 CAD나 실측값은 아닙니다.</p></section>
