...
export default function HR24Scene(p:Props){
 const host=useRef<HTMLDivElement>(null),live=useRef(p);useEffect(()=>{live.current=p;},[p]);const [error,setError]=useState('');
 useEffect(()=>{const el=host.current!;let renderer:T.WebGLRenderer;try{renderer=new T.WebGLRenderer({antialias:true,alpha:true});}catch{queueMicrotask(()=>setError('WebGL을 사용할 수 없습니다. 오른쪽 원본 도면으로 확인할 수 있습니다.'));return;}
 ...
 const scene=new T.Scene(),camera=new T.PerspectiveCamera(38,1,.01,10);camera.position.set(-1.2,.95,-1.5);const controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,.4,0);controls.enableDamping=true;controls.minDistance=.15;controls.maxDistance=3;
 ...
 }).catch(()=>{if(alive)setError('HR24B 모델 또는 배관 파일을 읽지 못했습니다.');});
 const grid=new T.GridHelper(2,20,0x58717b,0x314a56);scene.add(grid);
 ...
 let raf=0,lastView='rear',phase=0,fanSpeed=0,fanAngle=0,last=performance.now();const loop=()=>{const now=performance.now(),dt=Math.min(.1,(now-last)/1000);last=now;const v=live.current;
 if(v.view!==lastView){lastView=v.view;camera.position.set(...(v.view==='front'?[1.1,.9,1.5]:v.view==='back'?[0,.45,-1.9]:v.view==='left'?[ -1.9,.45,0]:v.view==='machine'?[-.55,.35,-.75]:[-1.2,.95,-1.5]) as [number,number,number]);controls.target.set(0,v.view==='machine'?.18:.4,v.view==='machine'?-.2:0);}
 ...
 // Evaporator fan runs with the circuit and stops while the door is open (service manual: EvapFM de-energizes when door is open).
