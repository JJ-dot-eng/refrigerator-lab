...
// pipeColor: optional colour of each circuit run along its length (fraction 0..1), e.g. refrigerant state;
// the pipes are repainted whenever colorKey changes. hideShell hides cabinet, doors and shelves entirely.
type PipeColor=(routeId:string,fraction:number)=>string|null;
type Props={src?:string;data?:SceneData|null;fitKey?:string;selected:string;onSelect:(id:string)=>void;view:string;transparent:boolean;flow:boolean;door:boolean;
 pickTarget?:string|null;onPick?:(mm:V3,normal:V3)=>void;markers?:{mm:number[];label:string}[];hideShell?:boolean;pipeColor?:PipeColor;colorKey?:string};
...
const VIEWS:Record<string,[number,number,number]>={rear:[-1.49,.68,-1.86],front:[1.37,.62,1.86],back:[0,.06,-2.36],left:[-2.36,.06,0],top:[.001,2.4,.001]};
...
type Stage={scene:T.Scene;camera:T.PerspectiveCamera;controls:OrbitControls;content:T.Group|null;markers:T.Group;items:T.Mesh[];spinners:{pivot:T.Group;axis:T.Vector3}[];routes:{curve:T.CurvePath<T.Vector3>;len:number;dots:T.Mesh[]}[];hinges:{group:T.Group;swing:number}[];grid:T.GridHelper|null;
 fit:{center:T.Vector3;size:number;machine:T.Vector3|null;machineSize:number}|null;fittedKey:string;lastView:string;lastSelected:string};
...
function applyView(s:Stage,view:string,selected=''){const f=s.fit;if(!f)return;s.lastView=view;s.lastSelected=selected;
 // "selected": frame whatever carries the selected id (a part group or a pipe run).
 if(view==='selected'){const box=new T.Box3();for(const m of s.items)if(m.userData.id===selected)box.expandByObject(m);
  if(!box.isEmpty()){const c=box.getCenter(new T.Vector3()),size=Math.max(...box.getSize(new T.Vector3()).toArray());
   s.camera.position.copy(c).add(new T.Vector3(-.6,.45,-.66).normalize().multiplyScalar(Math.max(size*2.2,f.size*.18)));s.controls.target.copy(c);return;}}
...
// Colour every pipe vertex by its position along the run (nearest point on the run's centre line).
function paintPipes(s:Stage,color?:PipeColor){
 const c=new T.Color();
 for(const m of s.items){const frac=m.userData.frac as Float32Array|undefined;if(!frac)continue;const mat=m.material as T.MeshStandardMaterial,attr=m.geometry.getAttribute('color');
  mat.color.setHex(color?0xffffff:m.userData.baseColor);
  for(let i=0;i<attr.count;i++){const v=color?.(m.userData.route,frac[i]);if(v)c.set(v);else if(color)c.setHex(m.userData.baseColor);else c.setRGB(1,1,1);attr.setXYZ(i,c.r,c.g,c.b);}
  attr.needsUpdate=true;}
}
function fractions(pos:T.BufferAttribute,pts:T.Vector3[]){
 const cum=[0];for(let i=1;i<pts.length;i++)cum.push(cum[i-1]+pts[i].distanceTo(pts[i-1]));const total=cum.at(-1)||1,out=new Float32Array(pos.count),v=new T.Vector3(),seg=new T.Line3(),q=new T.Vector3();
 for(let i=0;i<pos.count;i++){v.fromBufferAttribute(pos,i);let best=Infinity,f=0;
  for(let k=1;k<pts.length;k++){seg.set(pts[k-1],pts[k]);const t=seg.closestPointToPointParameter(v,true);seg.at(t,q);const d=q.distanceToSquared(v);if(d<best){best=d;f=(cum[k-1]+t*(cum[k]-cum[k-1]))/total;}}
  out[i]=f;}
 return out;
}
function setContent(s:Stage,data:SceneData,key:string,view:string,pipeColor?:PipeColor){
 ...
 const runPts=Object.fromEntries(data.routes.routes.map(r=>[r.id,r.points.map(cadTo3)]));
 for(const part of data.model.parts){const id=part.group||part.id,hinge=hingeOf[id]??null;const pivot=part.spin?cadTo3(part.spin.centerMm):null;const positions:number[]=[];
  ...
  const pipe=part.kind==='pipe',route=part.id.replace(/_pipe$/,''),mesh=new T.Mesh(geo,new T.MeshStandardMaterial({color:part.color,metalness:pipe?.6:.35,roughness:pipe?.35:.5,side:T.DoubleSide,vertexColors:pipe&&!!runPts[route]}));mesh.userData.id=id;items.push(mesh);
  if(pipe&&runPts[route]){geo.setAttribute('color',new T.Float32BufferAttribute(new Float32Array(geo.getAttribute('position').count*3).fill(1),3));Object.assign(mesh.userData,{route,baseColor:part.color,frac:fractions(geo.getAttribute('position') as T.BufferAttribute,runPts[route])});}
 ...
 s.scene.add(content);Object.assign(s,{content,items,spinners,routes,hinges});paintPipes(s,pipeColor);
...
export default function ModelScene(p:Props){
 ...
 const s:Stage={scene,camera,controls,content:null,markers,items:[],spinners:[],routes:[],hinges:[],grid:null,fit:null,fittedKey:'',lastView:'',lastSelected:''};stage.current=s;
 ...
 if(s.fit&&(v.view!==s.lastView||(v.view==='selected'&&v.selected!==s.lastSelected)))applyView(s,v.view,v.selected);
 for(const h of s.hinges)h.group.rotation.y=v.door?h.swing*Math.PI/2:0;
 for(const m of s.items){const id=m.userData.id;const shell=isShell(id),mat=m.material as T.MeshStandardMaterial;m.visible=!(shell&&v.hideShell);mat.transparent=shell&&v.transparent;mat.opacity=shell&&v.transparent?.08:1;mat.depthWrite=!mat.transparent;mat.emissive.setHex(id===v.selected?0x1d5550:0);}
 ...
 useEffect(()=>{const key=live.current.fitKey??p.src??'';
  if(p.data){if(stage.current)setContent(stage.current,p.data,key,live.current.view,live.current.pipeColor);return;}
  ...
  Promise.all([fetch(`${p.src}/model.json`).then(r=>{if(!r.ok)throw Error();return r.json() as Promise<SceneData['model']>;}),fetch(`${p.src}/routes.json`).then(r=>{if(!r.ok)throw Error();return r.json() as Promise<SceneData['routes']>;})])
   .then(([model,routes])=>{if(alive&&stage.current){setError('');setContent(stage.current,{model,routes},key,live.current.view,live.current.pipeColor);}}).catch(()=>{if(alive)setError('모델 또는 배관 파일을 읽지 못했습니다.');});
  return()=>{alive=false;};},[p.src,p.data]);
 useEffect(()=>{if(stage.current)paintPipes(stage.current,live.current.pipeColor);},[p.colorKey]);
