...
export type SceneData={model:{doorPivotMm:number[]|null;parts:Part[]};routes:{routes:Route[]}};
...
type Stage={scene:T.Scene;camera:T.PerspectiveCamera;controls:OrbitControls;content:T.Group|null;markers:T.Group;items:T.Mesh[];spinners:{pivot:T.Group;axis:T.Vector3}[];routes:{curve:T.CurvePath<T.Vector3>;len:number;dots:T.Mesh[]}[];hinge:T.Group|null;grid:T.GridHelper|null;
...
function setContent(s:Stage,data:SceneData,key:string,view:string){
 ...
 const hinge=data.model.doorPivotMm?new T.Group():null;if(hinge){hinge.position.copy(cadTo3(data.model.doorPivotMm!));content.add(hinge);}
 for(const part of data.model.parts){const id=part.group||part.id,isDoor=id==='door'&&!!hinge;const pivot=part.spin?cadTo3(part.spin.centerMm):null;const positions:number[]=[];
  for(let i=0;i<part.positions.length;i+=3){const v=cadTo3(part.positions.slice(i,i+3));if(isDoor)v.sub(hinge!.position);if(pivot)v.sub(pivot);positions.push(v.x,v.y,v.z);}
  ...
  if(pivot&&part.spin){const g=new T.Group();g.position.copy(pivot);g.add(mesh);content.add(g);const a=part.spin.axisMm;spinners.push({pivot:g,axis:new T.Vector3(a[0],a[2],-a[1]).normalize()});}else (isDoor?hinge!:content).add(mesh);}
 ...
 s.scene.add(content);Object.assign(s,{content,items,spinners,routes,hinge});
...
export default function ModelScene(p:Props){
 ...
 const s:Stage={scene,camera,controls,content:null,markers,items:[],spinners:[],routes:[],hinge:null,grid:null,fit:null,fittedKey:'',lastView:''};stage.current=s;
 ...
 const ray=new T.Raycaster(),mouse=new T.Vector2();let down=[0,0];const pd=(e:PointerEvent)=>{down=[e.clientX,e.clientY]},pu=(e:PointerEvent)=>{if(Math.hypot(e.clientX-down[0],e.clientY-down[1])>5)return;const b=el.getBoundingClientRect();mouse.set((e.clientX-b.left)/b.width*2-1,1-(e.clientY-b.top)/b.height*2);camera.updateMatrixWorld();ray.setFromCamera(mouse,camera);const v=live.current;
  ...
  const hit=ray.intersectObjects(s.items.filter(m=>m.visible&&!(v.transparent&&SHELL.includes(m.userData.id))))[0];if(hit)v.onSelect(hit.object.userData.id);};el.addEventListener('pointerdown',pd);el.addEventListener('pointerup',pu);
 ...
 if(s.hinge)s.hinge.rotation.y=v.door?Math.PI/2:0;
 for(const m of s.items){const id=m.userData.id;const shell=SHELL.includes(id),mat=m.material as T.MeshStandardMaterial;mat.transparent=shell&&v.transparent;mat.opacity=shell&&v.transparent?.08:1;mat.depthWrite=!mat.transparent;mat.emissive.setHex(id===v.selected?0x1d5550:0);}
