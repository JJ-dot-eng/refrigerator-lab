...
type Stage={scene:T.Scene;camera:T.PerspectiveCamera;controls:OrbitControls;content:T.Group|null;items:T.Mesh[];spinners:{pivot:T.Group;axis:T.Vector3}[];routes:{curve:T.CurvePath<T.Vector3>;len:number;dots:T.Mesh[]}[];hinge:T.Group|null;grid:T.GridHelper|null;
...
export default function ModelScene(p:Props){
 ...
 const s:Stage={scene,camera,controls,content:null,items:[],spinners:[],routes:[],hinge:null,grid:null,fit:null,fittedKey:'',lastView:''};stage.current=s;
 ...
 useEffect(()=>{const key=p.fitKey??p.src??'';
  ...
  return()=>{alive=false;};},[p.src,p.data,p.fitKey]);
